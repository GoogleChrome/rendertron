...
import {makeExpressMiddleware} from '../express-middleware';
...
function makeApp(options: Options) {
  return express()
      .use(makeExpressMiddleware(options))
...
test('makes a middleware function', async (t) => {
  const m = makeExpressMiddleware({proxyUrl: 'http://example.com'});
  t.truthy(m);
});

test('throws if no proxyUrl given', async (t) => {
  const makeMiddlewareUntyped = makeExpressMiddleware as (options?: unknown) =>
                                    express.Application;
