...
export class Renderer {
  ...
  async serialize(req: SerializeRequest.AsObject): Promise<SerializeResponse> {
    ...
    const resHeaders: Map<string, string> = customHeaders ? new Map(JSON.parse(customHeaders)) : new Map()
  ...
  private async setupPage(conf: PageConfig.AsObject | undefined): Promise<puppeteer.Page> {
    ...
    const userAgent = conf.isMobile ? conf.userAgent || MOBILE_USERAGENT : conf.userAgent
  ...
  private static waitUntilFactory(req: WaitUntil): puppeteer.PuppeteerLifeCycleEvent {
    switch (req) {
      case WaitUntil.WAIT_UNTIL_LOAD: return "load"
      case WaitUntil.WAIT_UNTIL_DOMCONTENT_LOADED: return "domcontentloaded"
      case WaitUntil.WAIT_UNTIL_NET_IDLE_0: return "networkidle0"
      case WaitUntil.WAIT_UNTIL_NET_IDLE_2: return "networkidle2"
      default: return "networkidle0"
    }
  }

  private static typeFactory(req: ScreenshotType): ScreenshotOptions["type"] {
    switch (req) {
      case ScreenshotType.SCREENSHOT_TYPE_JPEG: return "jpeg"
      case ScreenshotType.SCREENSHOT_TYPE_PNG: return "png"
      case ScreenshotType.SCREENSHOT_TYPE_WEBP: return "webp"
      default: return "jpeg"
    }
  }

  private static encodingFactory(req: ScreenshotEncoding): ScreenshotOptions["encoding"] {
    switch (req) {
      case ScreenshotEncoding.SCREENSHOT_ENCODING_BASE64: return "base64"
      case ScreenshotEncoding.SCREENSHOT_ENCODING_BINARY: return "binary"
