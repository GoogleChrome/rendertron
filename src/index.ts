...
import {IRendertronServiceServer, RendertronServiceService} from "../generated/nodejs/nemoengineering/rendertron/v1/rendertron_grpc_pb"
    ...
    async initialize() {
        ...
        const service: IRendertronServiceServer = {
            screenshot: (call, callback) => {
                this.renderer?.screenshot(call.request.toObject())
            ...
            serialize: (call, callback) => {
                this.renderer?.serialize(call.request.toObject())
                .then(res => callback(null, res))
                .catch(err => {
                    if (err instanceof PageError) {
                        callback({
